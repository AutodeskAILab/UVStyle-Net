...
import platform
import random
from datasets.font_util import valid_font
...
def _collate_graphs_with_images(batch):
...
class WireMNIST(Dataset):
    def __init__(
        ...
        in_memory=True,
        apply_line_symmetry=0.0,
    ):
        """
        Load and create the WireMNIST dataset
    ...
    def get_dataloader(self, batch_size, shuffle):
        return DataLoader(
            self, batch_size=batch_size, shuffle=shuffle, collate_fn=_collate_graphs
        )


class WireMNISTWithImages(Dataset):
    def __init__(
        self,
        root_dir,
        img_root_dir,
        ...
        """
        Load and create the WireMNIST dataset augmented with image renderings of the curve-networks
        :param root_dir: Root path to the dataset of curve-networks
        :param img_root_dir: Root path to the dataset of images
        :param train: Whether train or test set
        :param size_percentage: Percentage of data to load per category. Given in range [0, 1].
        :param in_memory: Whether to keep the entire dataset in memory (This is always done in Windows)
        :param apply_line_symmetry: Probability of applying a line symmetry transform on the curve grid
        ...
        self.in_memory = in_memory
        path = pathlib.Path(root_dir)
        img_path = pathlib.Path(img_root_dir)
        if split == "train" or split == "val":
            subfolder = "train"
        else:
            subfolder = "test"
        ...
        img_path /= subfolder
        self.graph_files = list([x for x in path.glob("*.bin") if valid_font(x)])
        self.images_files = list([x for x in img_path.glob("*.png") if valid_font(x)])

        print(
            "Found {} {} {} data.".format(
                len(self.graph_files), len(self.images_files), subfolder
            )
        )
        self.labels = []
        ...
        for file_name in self.images_files:
            query_name = file_name.name.split(".")[0]
        ...
        print(
            "Loaded {} face-adj graphs and {} image files from {} classes".format(
                len(self.graph_files), len(self.image_files), self.num_classes
            )
        )
        self.apply_line_symmetry = apply_line_symmetry
    ...
    def get_dataloader(self, batch_size, shuffle):
        return DataLoader(
            ...
            shuffle=shuffle,
            collate_fn=_collate_graphs_with_images,
        )
