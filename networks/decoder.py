...
class _mlpAdj(nn.Module):
    def __init__(self, nlatent=1024):
        """Atlas decoder"""

        super(_mlpAdj, self).__init__()
...
class AtlasV2_Decoder(nn.Module):
    def __init__(self, num_points, emb_dims):
        ...
        self.decoder = nn.ModuleList(
            [_mlpAdj(nlatent=2 + self.nlatent) for i in range(0, self.nb_primitives)]
        )
    ...
    def forward(self, x):
        ...
        for i in range(0, self.nb_primitives):
            if self.training == True:
                ...
                grid = self.grids[i]
                rand_grid = grid.unsqueeze(0).expand(
                    x.size(0), grid.size(0), grid.size(1)
                )
