...
def conv1d(in_channels, out_channels, **kwargs):
    ksize = kwargs.get("kernel_size", 1)
    padding = kwargs.get("padding", 0)
    bias = kwargs.get("bias", False)
    return nn.Sequential(
        nn.Conv1d(
            in_channels, out_channels, kernel_size=ksize, padding=padding, bias=bias
        ),
...
def conv3d(in_channels, out_channels, kernel_size, padding=0, bias=False, **kwargs):
    return nn.Sequential(
        nn.Conv3d(
            in_channels,
            out_channels,
